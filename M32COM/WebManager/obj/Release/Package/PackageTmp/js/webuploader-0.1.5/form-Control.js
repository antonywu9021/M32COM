...
var FileHandle =
   {
       initOpts: function (options) {
           var defaults = {
               ...
               EnableEdit: false,//是否启用编辑
               filetips: "",//提示内容，类似于（建议上传600*300之类的）
               defaultUrl: "",//编辑时显示的图片路径（只正对于单个文件）
               imgStyle: ""
           };
           ...
           FileAccept = {
               title: 'office文件',
               extensions: 'doc,docx,xls,xlsx,pdf,ppt,pptx,txt,gif,jpg,jpeg,bmp,png,zip,rar',
       ...
       delFile: function (uploader, fileid, identity) {
           ...
           $.post("/UpLoad/delimg", { 'filepathname': $("#WUimg_" + fileid).attr("filevalue") }, function (returndata) {
       ...
       delFileEdit: function (fileid, url) {
           ...
           $.post("/UpLoad/delimg", { 'filepathname': url }, function (returndata) {
               if (returndata.result == "true") {
                   $("ul.forminfo").find("#child_" + fileid).remove();
                   $.alert("删除成功！");
               }
               ...
               if (opts.defaultUrl != null) {
                   if (opts.defaultUrl !== "") {
                       ...
                       var html = "<div id=\"child_" + contentId + "\" class=\"dz-preview dz-file-preview\">"
                           ...
                           + "</div>"
                           + "<div class=\"tools\">" + (!opts.EnableEdit ? "" : "<a  id=\"WUFileEdit_" + contentId + "\">编辑</a>&nbsp;&nbsp;") + "<a onclick=\"FileHandle.delFileEdit('" + contentId + "','" + opts.defaultUrl + "');\" id=\"WUFileDel_" + contentId + "\" style=\"cursor: pointer\" >删除</a></div>"
                           + FileHtml
                           + "</div>";
    ...
    function initWebUpload(target, options) {
        ...
        uploader.on('fileQueued', function (file) {
            ...
            var html = "<div id=\"child_" + file.id + "\" class=\"dz-preview dz-file-preview dz-processing\">"
                       ...
                       + "</div>"
                       + "<div class=\"tools\">" + (!opts.EnableEdit ? "" : "<a  id=\"WUFileEdit_" + file.id + "\">编辑</a>&nbsp;&nbsp;") + "<a id=\"WUFileDel_" + file.id + "\" style=\"cursor:pointer\" >删除</a></div>"
                       + FileHtml
            + "</div>";
        ...
        uploader.on("error", function (type) {
            if (type == "F_DUPLICATE") {
                ...
                $.alert("系统提示: 所选附件总大小不可超过" + allMaxSize + "M 哦！");
            }
        ...
        uploader.on('uploadSuccess', function (file, response) {
            ...
            if ($(".col-md-12").html() == "") {
                var mm = "<div id=\"child_WU_FILE_0\" class=\"dz-preview dz-file-preview dz-processing dz-success\"><div class=\"dz-details\"><div class=\"dz-filename\"><span data-dz-name=\"\">" + response.filename + "</span><span style=\"display: block;margin-top: 2px;color: #128C11;\">上传成功！</span><span></span></div><div class=\"dz-size\" data-dz-size=\"\"><strong>" + WebUploader.formatSize(file.size) + "</strong></div><img data-dz-thumbnail=\"\" id=\"WUimg_WU_FILE_0\" src=\"" + response.filePath + "\" title=\"" + response.filename + "\" filevalue=\"" + response.filevalue + "\" style=\"display: inline;\"></div><div class=\"dz-progress\"><span class=\"dz-upload\" data-dz-uploadprogress=\"\" style=\"width: 100%;\"></span></div><div class=\"dz-success-mark\"><span style=\"color: #0EDE0B;font-size: 30px;\">✔</span></div><div class=\"dz-success-message\"><span data-dz-successmessage=\"\"></span></div><div class=\"dz-error-mark\"><span style=\"color: #F10A06;font-size: 30px;\">✘</span></div><div class=\"dz-error-message\"><span data-dz-errormessage=\"\"></span></div><div class=\"tools\"><a id=\"WUFileDel_WU_FILE_0\" style=\"cursor:pointer\">删除</a></div><input type=\"hidden\" id=\"WUOPath_WU_FILE_0\" name=\"ImgUrl_1\" title=\"" + response.filename + "\" value=\"" + response.filevalue + "\"></div>";
                $(".col-md-12").html(mm);
            }
