...
(function($, undefined) {
    ...
    $.uedWidget = function(name, base, prototype) {
        ...
        // baseDefault为Widget类，组件Default会继承base类的所有方法
        if (!prototype) {
            prototype = base;
            base = $.UEDWidget;
        }
        ...
        $[ namespace ][name].prototype = $.extend(true, basePrototype, {
            namespace : namespace,
            widgetName : name,
            // 组件的事件名前缀，调用_trigger的时候会Default给trigger的事件加上前缀
        ...
        $.fn[name] = function(options) {
            ...
            if (isMethodCall) {
                ...
                this.each(function() {
                    var instance = $.data(this, name);
                    if (instance) {
                        // Set Upoptions后再次调用_init方法，第一次调用是在_createWidget方法里面。这个方法需要开发者去实现。
    ...
    $.UEDWidget.prototype = {
        ...
        // $.widget中优化过的trigger方法。type是回调事件的Name，如"onRowClick"，event是触发回调的事件（通常没有这个事件的时候传null）
