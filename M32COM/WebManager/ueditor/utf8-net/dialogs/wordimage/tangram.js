 ...
 * @see baidu.dom.q
 *
 * @return {HTMLElement|null} 获取的元素，查找不到时Returnnull,如果参数不合法，直接Return参数.
 */
 ...
 * @param {Object} [thisObject] 函数调用时的this指针，如果没有此参数，Default是当前遍历的数组
 * @remark
 ...
 * 在Browser identification最后AddVersion + 数字进行版本标识
 * opera后面的数字保持在9.80不变
 */
 ...
 * @param {string} position 插入html的位置InfoMation，取值为beforeBegin,afterBegin,beforeEnd,afterEnd
 ...
 * @param {string} 	options.base 				Set Up用于解析swf文件中的所有相对路径语句的Basic 目录或URL
 * @param {string} 	options.bgcolor 			swf文件的背景色
 * @param {string} 	options.salign 				Set Up缩放的swf文件在由width和heightSet Up定义的区域内的位置。允许值：l/r/t/b/tl/tr/bl/br
 ...
 * @param {string} 	options.quality 			Set Upflash播放的画质，允许值：low/medium/high/autolow/autohigh/best
 * @param {string} 	options.scale 				Set Upflash内容如何缩放来适应Set Up的宽高。允许值：showall/noborder/exactfit
 * @param {string} 	options.wmode 				Set Upflash的显示模式。允许值：window/opaque/transparent
 * @param {string} 	options.allowscriptaccess 	Set Upflash与页面的通信权限。允许值：always/never/sameDomain
 * @param {string} 	options.allownetworking 	Set Upswf文件中允许使用的网络API。允许值：all/internal/none
 * @param {boolean} options.allowfullscreen 	是否允许flash全屏。允许值：true/false
 * @param {boolean} options.seamlesstabbing 	允许Set Up执行无缝跳格，从而使User 能跳出flash应用程序。该参数只能在安装Flash7及更高版本的Windows中使用。允许值：true/false
 * @param {boolean} options.devicefont 			Set Up静态文本对象是否以设备字体呈现。允许值：true/false
...
baidu.flash._Base = (function(){
    ...
    return function(options, callBack){
         ...
         * @param {Function} [callBack] 异步调用后将Return值作为参数的调用回调函数，如无Return值，可以不传入此参数
         * @return {Null}
        */
 ...
 * @config {String} vars.uploadDataFieldName POST请求中图片数据的key,Default值'picdata'
 * @config {String} vars.picDescFieldName POST请求中图片描述的key,Default值'picDesc'
 ...
 * @config {Number} vars.mode mode == 0时，是使用滚动条，mode == 1时，拉伸flash, Default值为0
 ...
 * @param {Function|string} opt_callback 数据加载End 时调用的函数或函数名
 ...
 * @config {string} [charset] script的字符集
 * @config {string} [queryField] 服务器端callback请求字段名，Default为callback
...
baidu.sio.callByServer = /**@function*/function(url, callback, opt_options) {
    ...
    /*
     * Return一个函数，用于立即（挂在window上）或者超时（挂在setTimeout中）时执行
     */
 ...
 *             
 * @returns {JSON} 解析 Resultjson对象
 */
baidu.json.parse = function (data) {
    //2010/12/09：更新至不使用原生parse，不检测User 输入是否正确
